...
from .models import Documento, Especialidades, DadosMedico, is_medico, DatasAbertas
...
def cadastro_medico(request):
    ...
    if is_medico(request.user):
        messages.add_message(request, constants.INFO, 'Você já possui cadastro médico!!')
...
def abrir_horario(request):
    ...
    if not is_medico(request.user):
        messages.add_message(request, constants.WARNING, 'Somente médicos podem abrir horarios')
...
def consulta_area_medico(request, id_consulta):
    ...
    if request.method == "GET":
        ...
        return render(request, 'consulta_area_medica.html', {'consulta':consulta, 'documentos':documentos})   
    elif request.method == "POST":
        consulta = Consulta.objects.get(id=id_consulta)
...
def finalizar_consulta(request, id_consulta):
    ...
    if request.user != consulta.data_aberta.user:
        ...
        return redirect ('/medicos/abrir_horario/{id_consulta}')
...
#Fazer validação de segurança nos restantes pontos do código
